...
The model has two main intentions.
It is showing the diffusion of plug-in hybrid vehicles in suburban neighborhoods, based on income distribution, preference and driving patterns.
The second purpose is to show the effect this diffusion of plug-in hybrid vehicles has on an existing static grid without smart management of charging cycles.

## HOW IT WORKS

At first a random neighbourhood with roughly 1830 houses is made, populated with users and some of them, based on income are given a car. In parallel a fixed, hierarchical 3 level grid is created and houses are connected to it.

The main decision makers are users. The users decide to buy or not to buy a new car. But they also decide, mainly based on driving patterns, whether or not to buy an internal combustion engine or a plug-in hybrid car. The decision is tilted a little bit by personal preference.  
The driving patterns are generated by daily commute and long distance trips that users can take. The users influence each other to change each others preference towards electric vehicles.

The effect of plug-in hybrid vehicles on the grid are estimated through their maximum power they can draw from the grid and two coincidence factors. One represents the likelihood of each individual car charging at peak time and is dependent on the size of the battery. The other is the likelihood of all the cars charging at the same time and is dependent only on the total number of cars.
The overcapacity of each transformer is calculated and if the overcapacity is below 0 the color of this transformer is changed to red.

## HOW TO USE IT

Basic Buttons:
Profiler: to check the speed of the different functions
Setup-square: Sets the model up
go-once: runs one tick (=one month)
Go: runs the model

Sliders:
The functions of the sliders can be grouped into three groups, grid, social and economic.
Sliders for the Grid:
The sliders for the grid basically give the user control over the size of the overcapacity.
NHFactor: Capacity of Neighborhood transformer relative to total demand at the beginning.
TFFactor: Capacity of the medium level transformers in proportion to the NH-Transformer
LVFactor: Capacity of the low voltage lines in proportion to the NH-Transformer

Slider for social aspects.
These sliders give the user control over the level of wealth and the interactions between the users. 
NHRichFactor: sets the level of wealth in the neighborhood
average-number-friendships: sets the relative number of friends one user has 
impact-on-friendships: the impact those friendships have on preferences
size-of-area-influence: the area around an user that is affected by having a PHEV
user-area-impact: The impact a user has on his surrounding by driving a PHEV

Sliders for economic aspects
The economic aspects of the model mainly deal with tax incentives and the development of battery technology. 
tax_global:sets the tax break one gets from buying an PHEV car
global_battery_price_init: sets the initial battery price in Euro per kWh per year
battery_price_drop: sets by how much the price of batteries drop each year

## THINGS TO NOTICE

At first look at the animation, it shows quite clearly how the system is structured. Use different NHRichFactor settings to alter the initial car coverage and average income. Users are given a year of driving history so upon clicking 'GO' they will immediately start buying new cars if their current one is old enough. 
Keep track of the electric vehicle counter and demand as well as capacity graphs. Based on the different parameters of the grid, social and economic aspects the grid will encounter capacity problems at different stage.
In the end notice, that basing mostly on the economic consideration all users choose to adopt electric vehicles within a couple of years.

## THINGS TO TRY

Most of all explore different settings of the NHRichFactor that defines the average income in the neighbourhood. 
Try to think of scenarios that would increase electric vehicle coverage faster and blow the fuses early. Then, try an alternate scenario. What would keep that neighbourhood from adopting such cars so fast?
